...
Таким образом, работа предполагала изучение нормативно-правовой документации Университета, доступной на сайте, создание технического задание на разработку интеллектуальной системы. Подбор данных для дообучение и выбор подходящей модели для определения тональности отзывов, создание схемы базу данных, наполнение ее данными, описание пользовательских потоков, создание интерефейса пользователя, а на этапе ВКР - создание веб-ресурса, позволяющего тестировать интеллектуальную систему - нейросеть для анализа тональности.
...
Для данной практики и для ВКР было возможно выбрать разные варианты создания интеллектуальной системы. Поскольку был выбран фреймворк Django, то именно под его требования необходимо было создать базу данных PostgreSQL, т.к. у Django есть требование на наличие определенных полей данных, а также на их размер. По этой причине пришлось модифицировать первоначально созданную версию БД.
...
Если данные в таблице не будут меняться, то в Django можно объединить таблицы в одну, но поскольку база данных создается для образовательной системы, то возможны изменения предметов – удаление и добавление, так как образовательная структура подразумевает возможность изменений. По этой причине решено было создать справочную таблицу в модели БД.
...
Веб-сайт разрабатывается на языке программирования Python, с использованием фреймворка Django, база данных – SQL, согласно техническому заданию, приведеннному ранее.
Веб-ресурс на Django обычно составляет один проект, в котором есть разная функциональность, но может быть и одна. В нашем проекте есть несколько функций веб-ресурса, поэтому они согласно философии Django помещаются в папку [apps](https://github.com/AliyaBadmaeva/PDP/tree/main/itos/itos/apps) (приложения). Каждое приложение отвечает за определенную функцию веб-ресурса. У нас на данном этапе созданы приложения [accounts](https://github.com/AliyaBadmaeva/PDP/tree/main/itos/itos/apps/accounts) – отвечает за авторизацию пользователей и имеет визуальную часть – **страница «Авторизация»** – это страница аутентификации, где необходимо ввести логин и пароль. 

Следующее приложение внутри проекта [blog](https://github.com/AliyaBadmaeva/PDP/tree/main/itos/itos/apps/blog) – после входа любой из трех пользователей попадает на страницу сайта **«Главная»** – приветственная, на которой описывается информация о веб-ресурсе, на ней будет отображено предназначение веб-ресура – определение эмоциональной окраски, какие есть типы пользователей и какие у пользователей возможные действия.

Следующее приложение [dashboard](https://github.com/AliyaBadmaeva/PDP/tree/main/itos/itos/apps/dashboard) – это приложение отвечает только за редирект после входа согласно роли на дашборд. Это приложение включает три страницы:

* **Третья страница «Написать отзыв»** – страница написания отзыва студентом, на ней будет краткая памятка по загрузке отзыва – необходимо выбрать доступный и изученный в прошлом семестре предмет и написать отзыв не более 512 символов, дата и код отзыва будут генерироваться системой автоматически. Внизу страницы можно будет увидеть прошлые отзывы с датой и будет отображаться их количество. Мотивация для студента будет описана на странице. Для студента написание отзывов – это возможность после 10 отзывов получить мерч в деканате. Данная страница доступна студентам.
  
* **Четвертая страница «Загрузить отзывы»**  - страница для загрузки на сайт отзывов в Excel-формате менеджером, курирующим учебный процесс. Данная страница доступна менеджерам. На странице будет памятка о формате загрузки данных. Мотивация для менеджера загружать отзывы – это один из ключевых показателей эффективности.
  
* **Пятая страница «Статистика»** - страница для скачивания отчетов. Можно скачать отчеты с отзывами и проставленной нейросетью оценкой. Данная страница будет доступна преподавателям. Мотивация для преподавателей – отсле-живание эффективности обучения и его качества, может быть использовано для рейтингования преподавателей.
  
Остальные страницы принадлежат упомянутому ранее приложению blog:
...
В данном репозитории можно ознакомиться со сверстанными html-страницами (7 штук):

[Страница авторизации](https://github.com/AliyaBadmaeva/PDP/blob/main/itos/itos/templates/accounts/login.html)

[Главная](https://github.com/AliyaBadmaeva/PDP/blob/main/itos/itos/templates/blog/main.html)

[Разведочный анализ данных](https://github.com/AliyaBadmaeva/PDP/blob/main/itos/itos/templates/blog/eda.html)

[Технологии для NLP](https://github.com/AliyaBadmaeva/PDP/blob/main/itos/itos/templates/blog/nlp.html)

[Часто задаваемые вопросы](https://github.com/AliyaBadmaeva/PDP/blob/main/itos/itos/templates/blog/faq.html)

[О программе](https://github.com/AliyaBadmaeva/PDP/blob/main/itos/itos/templates/blog/about.html)

[Контакты](https://github.com/AliyaBadmaeva/PDP/blob/main/itos/itos/templates/blog/contacts.html)
